...
**現在のバージョン: v1.3.5 ✅完了** (ルートボーン非表示 + FBXファイル読み込みUI)
...
### 🎉 **ルートボーン第一階層の非表示 + FBX読み込みUI可視化**

**✅ 実装内容:**

1. **ルートボーン→第一階層ボーン接続の非表示** (`fbx-loader.js`)
   - `hideRootBoneConnections(helper)` メソッド追加
   - SkeletonHelperの`updateMatrixWorld`をオーバーライド
   - ルートボーン直下の子ボーンへの接続ラインをゼロ長にして非表示化
   - SkinnedMesh/ルートボーン両方のパスで適用

2. **FBXファイル読み込みUIの可視化** (`index.html`)
   - 非表示だったファイルインプットとチェックボックスを可視パネルに変更
   - `fbx_joint_fixed.html`のUIスタイルを参考に実装
   - 左下にファイル選択UI + Z-up→Y-up変換チェックボックスを配置

3. **UI制御の統合** (`camera-controls.js`)
   - ファイルインポートUIをポインターロック時透明度制御に追加
   - クリックによるポインターロック復帰からファイルUIを除外
