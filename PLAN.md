...
### 🔍 **glTFカメラアニメーション出力問題**（最優先修正事項）

**問題の詳細:**
- **症状**: [U]キーでカメラモーション書き出し実行時、空のglTFファイルが出力される
- **継続期間**: 初回実装時から一度も正常出力に成功していない
- **出力内容**: 
```json
{
  "asset": {
...
**欠如している要素:**
- `cameras` セクション（カメラオブジェクト定義）
- `animations` セクション（位置・回転のキーフレーム）
- `accessors`, `bufferViews`, `buffers` セクション（アニメーションデータ）

**推定原因:**
1. **カメラオブジェクト未追加**: Three.jsカメラオブジェクトがglTF出力対象に含まれていない
2. **アニメーションデータ未設定**: 記録されたカメラモーションがThree.js AnimationClip形式に変換されていない
3. **GLTFExporter設定問題**: カメラ・アニメーション出力オプションが無効化されている
4. **データ構造不整合**: motion-capture.jsで記録したデータ形式とGLTFExporter期待形式の不一致

**要調査・修正項目:**
1. **motion-capture.js**: 
   - カメラモーション記録データの形式確認
   - AnimationClip生成処理の実装状況
   - GLTFExporter呼び出し時の引数・オプション
2. **GLTFExporter動作**:
   - カメラオブジェクトの出力対象追加
   - アニメーション出力オプションの有効化
   - 出力データの詳細デバッグ
