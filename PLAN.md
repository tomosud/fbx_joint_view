...
- **視点依存性**（重要）:
  ...
  - **原点側からキャラを見て近づく** → 消失発生
- **ジョイント動作**:
  - キャラが移動・アニメーション中でも**ジョイントルートは常に原点(0,0,0)に固定**
- **現象**: ジョイント付きモデルが突然全身消失、遠ざかるまで復帰しない

**技術的推察:**
- **バウンディングボックス異常**: 
  - ジョイントルート(原点)とキャラ位置が離れることで巨大なバウンディングボックス生成
  - 原点-キャラ間の直線距離がボックスサイズに影響
- **視点依存フラスタムカリング**: 
  - 原点側から見る = 巨大バウンディングボックスの内部から見る状態
  - カメラがボックス内部に入ると誤ってカリング（消失）
  - エリア外側から見る = ボックス外部から見るため正常表示
- **ジョイント座標系の問題**: 
  - FBXのジョイント座標系とメッシュ座標系の不整合
  - SkeletonHelperが原点基準でボーン表示している影響

**要調査項目:**
1. **バウンディングボックス検証**: 
   - FBXオブジェクトの実際のバウンディングボックスサイズ確認
   - ジョイント含有時のボックス計算ロジック
2. **フラスタムカリング検証**:
   - カメラ位置と視点方向によるカリング動作
   - 巨大バウンディングボックス内部からの視点での挙動
3. **ジョイント座標系調査**:
   - SkeletonHelperの原点固定がメインオブジェクトに与える影響
   - FBXのボーン座標系とメッシュ座標系の関係
4. **視点依存性の詳細検証**:
   - 原点側 vs エリア外側からの視点での差異
   - カメラ移動パスと消失タイミングの関係
